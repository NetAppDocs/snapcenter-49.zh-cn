...
NOTE: 特殊字符"`%`" is not supported in the custom path for the repository database. If you include "`%`"、安装失败。
...
+
日志文件始终列在管理员用户的 %temp% 文件夹中（最早的先列出）。如果要重定向日志位置、请从命令提示符处运行以下命令来启动SnapCenter 服务器安装：``C:\installer_location\installer_name.exe /log"C:\"``
